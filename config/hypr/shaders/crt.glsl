...
// Thanks to: https://github.com/Alexander-Chudnikov-Git
...
const float CURVE = 5.0;
const float SMOOTH = 0.004;
const float SHINE = 0.12;

const vec2 RESOLUTION = vec2(2880, 1800);
...
{    
    ...
    if (uvContent.x > 0. && uvContent.x < 1. && uvContent.y > 0. && uvContent.y < 1.)
    {
        ...
        col += col * vec3(random(uvContent)) * N_OPASITY;
        col += col * sin(110.0) * FLICKERING;
        content += col;
    }
